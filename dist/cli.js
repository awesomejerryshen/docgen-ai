...
program
    ...
    try {
        ...
        const result = await (0, generator_1.generateDocs)(targetPath, options);
        if (options.dryRun) {
...
program
    ...
    .action(async (targetPath, options) => {
    try {
        await (0, generator_1.previewDocs)(targetPath, { output: './docs', template: 'default', dryRun: true, apiKey: options.apiKey });
    }
    catch (error) {
        console.error('❌ Error:', error instanceof Error ? error.message : error);
        process.exit(1);
